...
TEST(BPlusTreeConcurrentTestC1, ScaleTestC1) {
  ...
  std::unique_ptr<DiskManager> disk_manager(new DiskManager("test.db"));
  std::unique_ptr<BufferPoolManager> bpm(new BufferPoolManagerInstance(5, disk_manager.get()));
  // create b+ tree
  BPlusTree<GenericKey<8>, RID, GenericComparator<8>> tree("foo_pk", bpm.get(), comparator);
  GenericKey<8> index_key;
  RID rid;
  // create transaction - use unique_ptr for exception safety
  std::unique_ptr<Transaction> transaction(new Transaction(0));
  // create and fetch header_page
  page_id_t page_id;
  auto header_page = bpm->NewPage(&page_id);
  (void)header_page;
  ...
  bpm->UnpinPage(HEADER_PAGE_ID, true);
  // All resources will be automatically deleted by unique_ptr
  remove("test.db");
  remove("test.log");
}
}  // namespace bustub